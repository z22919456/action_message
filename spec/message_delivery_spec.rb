...
describe ActionShortMessage::MessageDelivery do
  let(:message_class) { BaseMessage }
  let(:action) { :welcome_with_two_arguments }
  let(:args) { ['John Doe', 'johnshoes.com'] }
  subject { ActionShortMessage::MessageDelivery.new(message_class, action, *args) }
  ...
  context 'instance methods' do
    ...
    it '#deliver_later' do
      job = double
      expect(ActionShortMessage::DeliveryJob).to receive(:set).and_return(job)
