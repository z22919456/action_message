...
describe ActionShortMessage::Adapters::Base do
  let(:credentials) { { something: 'this' } }
  subject { ActionShortMessage::Adapters::Base.new(credentials) }
  ...
  context 'instance methods' do
    ...
    describe '#senders' do
      ...
      it 'prioritizes params provided sender' do
        allow(ActionShortMessage::Base).to receive_message_chain(:default_params, :[], :from).and_return(config_sender)
      ...
      it 'uses configured sender if no sender is sent via params' do
        expect(ActionShortMessage::Base).to receive_message_chain(:default_params, :[]).and_return(config_sender)
